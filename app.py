...
def change_password_logic():
    ...
    if (users[0][2] == currentPassword):
        if newPassword == reTypePassword:
            cursor.execute("""UPDATE users SET password='{}' where user_id={}""".format(
                newPassword, user_id))
...
def admin_login():
    if request.method == 'POST':
        ...
        cursor = conn.cursor()
        cursor.execute(
            "SELECT * FROM users WHERE email=%s AND password=%s AND role='admin'", (email, password))
        user = cursor.fetchone()
        cursor.close()
...
def predict():
    if request.method == 'POST':
        ...
        if len(loan_data) > 0:
            flash(
                'You have already taken loan...Please complete your due to apply again!', 'error')
            return redirect("/predict")
        ...
        X = [credit,  LoanAmountlog, ApplicantIncomelog, Loan_Amount_Termlog, totalincomelog, male,
             married_yes, dependents_1, dependents_2, dependents_3, not_graduate, employed_yes, semiurban, urban]
